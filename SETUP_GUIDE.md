...
**Records do not survive on the free plan.** The blueprint sets `plan: free`,
and a free instance has no persistent disk and is stopped after a spell of
inactivity. Each deploy, and each wake from idle, rebuilds the container from
the image, so `ticket_records.xlsx` and `booking_counter.json` are back to
whatever the image shipped:

- Bookings recorded since the last deploy are **gone**, and there is no route
  that serves the tracker, so they cannot be exported first.
- The booking-ID counter re-seeds from the (now stale) tracker, so **IDs are
  handed out again** and can collide with ones already given to customers.

To keep them, move to the `starter` plan and give the service a disk. The
render.yaml comment carries the exact block; it mounts at `/var/data` and
`DATA_DIR` points the two files there. The disk starts **empty**, so copy an
existing `ticket_records.xlsx` in from the Render shell — `.dockerignore`
