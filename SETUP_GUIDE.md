...
1. Render → **New → Blueprint** → pick this repo. It creates a Docker web
   ...
   `/api/health`.
2. Set the two secrets in the dashboard (they are marked `sync: false`, so they
   are never committed): `WHATSAPP_ACCESS_TOKEN` and
   `WHATSAPP_PHONE_NUMBER_ID`. `SECRET_KEY` is generated by Render.
3. Deploy. `GET /api/health` should return `"ocr": true` and `"whatsapp": true`.
